...
protected:
    ...
    void checkMesh(size_t refCount, vector3d refmin, vector3d refmax, std::string fileName = "")
    {
        ...
        vector3d maximum(0,0,0);
        vector3d minimum(500,500,500);

        for (size_t j = 0; j < desc.layout[0].itemsCount; j += 3)
        {
            vector3d current(&desc.layout[0].items[j]);
...
{
    m_builder->SetElement(ElementType::Cube, Vector3D(0,0,0), Directions::pY, true);
    ASSERT_NO_FATAL_FAILURE(checkMesh(36, vector3d(0,0,0), vector3d(1,1,1), "c:\\tmp\\cube.obj"));
}
...
TEST_F(BuildingBerthTest, ElementsMesh)
{
    ...
    ASSERT_NO_FATAL_FAILURE(checkMesh(36 * 3, vector3d(0,0,0), vector3d(1,5,1), "c:\\tmp\\cube_pillar.obj"));
}
...
TEST_F(BuildingBerthTest, SinglePillarMesh)
{
    ...
    ASSERT_NO_FATAL_FAILURE(checkMesh(84, vector3d(0,0,0), vector3d(1,3,1), "c:\\tmp\\united_pillar.obj"));
}
...
TEST_F(BuildingBerthTest, CubeMesh)
{
    ...
    ASSERT_NO_FATAL_FAILURE(checkMesh(cubeScales*cubeScales*6*6, vector3d(0,0,0), vector3d(BBox, BBox, BBox), "c:\\tmp\\HugeCube.obj"));
}
...
TEST_F(BuildingBerthTest, SpongeMesh)
{
    ...
    const float BBox = (float)cubeScales;
    //hmmm... 64? * 64?
    ASSERT_NO_FATAL_FAILURE(checkMesh((cubeScales*cubeScales*cubeScales/2)*36, vector3d(0,0,0), vector3d(BBox, BBox, BBox), "c:\\tmp\\sponge.obj"));
}
